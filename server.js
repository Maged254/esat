...
// ── Casual PPE Requests ──────────────────────────────────────────────────
const CASUAL_PPE_VIEW_ROLES = ['admin', 'ehs_manager', 'ehs_officer', 'supervisor', 'scm_officer'];
...
app.post('/api/casual-ppe-requests', auth, async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      if (!item.ppe_item_id) continue;
      const { rows } = await client_db.query(
        `INSERT INTO casual_ppe_requests (casual_id, ppe_item_id, size_value, quantity, flagged_by)
         VALUES ($1,$2,$3,$4,$5) RETURNING *`,
        [casual_id, item.ppe_item_id, item.size_value || null, item.quantity || 1, req.user.id]
      );
      inserted.push(rows[0]);
    }
    await client_db.query('COMMIT');
    res.json(inserted);
  } catch(e) { await client_db.query('ROLLBACK'); console.error('Casual PPE request create error:', e.message); res.status(500).json({ error: e.message }); }
  finally { client_db.release(); }
});

// List casual PPE requests (project-scoped via casual's project)
app.get('/api/casual-ppe-requests', auth, async (req, res) => {
  if (!CASUAL_PPE_VIEW_ROLES.includes(req.user.role)) return res.status(403).json({ error: 'Not authorized' });
  try {
    const { rows } = await pool.query(`
      SELECT r.*,
        c.full_name as casual_name, c.national_id as casual_national_id, c.project, c.client, c.employment_status,
      ...
      FROM casual_ppe_requests r
      JOIN casuals c ON c.id = r.casual_id
    ...
    if (casualProjects !== null) {
      if (casualProjects.length === 0) filteredRows = [];
      else filteredRows = rows.filter(r => casualProjects.includes(r.project));
    }
    res.json(filteredRows);
  } catch(e) { console.error('Casual PPE requests error:', e.message); res.status(500).json({ error: e.message }); }
});

// Update casual PPE request status
app.put('/api/casual-ppe-requests/:id/status', auth, async (req, res) => {
  const { status, distribution_method, courier_tracking_number } = req.body;
  const allowedRoles = ['admin', 'ehs_manager', 'scm_officer'];
  ...
  try {
    ...
    const { rows } = await client_db.query(
      'UPDATE casual_ppe_requests SET status=$1' + extraFields + ', updated_at=NOW() WHERE id=$2 RETURNING *',
      extraParams
    );
    if (!rows.length) {
    ...
    await client_db.query('COMMIT');
    res.json(rows[0]);
  } catch(e) { await client_db.query('ROLLBACK'); console.error(e); res.status(500).json({ error: e.message }); }
  finally { client_db.release(); }
});
