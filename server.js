...
app.get('/api/ncr', auth, async (req, res) => {
  try {
    const { rows } = await pool.query(`SELECT n.*,e.full_name as employee_name,e.employee_number,e.project,p.name as ppe_name,p.category,u.full_name as audited_by_name FROM ncr_items n JOIN employees e ON e.id=n.employee_id JOIN ppe_items p ON p.id=n.ppe_item_id LEFT JOIN audit_items ai ON ai.id=n.audit_item_id LEFT JOIN audits a ON a.id=ai.audit_id LEFT JOIN users u ON u.id=a.audited_by ORDER BY n.created_at DESC`);
    res.json(rows);
...
app.get('/api/ppe-requests', auth, async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT r.*,
        e.full_name as employee_name, e.employee_number, e.national_id as employee_national_id, e.employment_status,
...
app.put('/api/ppe-requests/:id/status', auth, async (req, res) => {
  ...
  try {
    ...
    if (status === 'distributed') extraFields = ', date_distributed=NOW(), distributed_by=$3, date_available=COALESCE(date_available,NOW()), available_by=COALESCE(available_by,$3), date_ordered=COALESCE(date_ordered,NOW()), ordered_by=COALESCE(ordered_by,$3)';
